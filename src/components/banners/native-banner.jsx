...
export const NativeBanner = ({ id, src, title, description }) => {
      ...
      <div className="card group bg-base-100 w-full shadow-md overflow-hidden rounded-xl relative border border-[color-mix(in_oklab,var(--color-base-content)_10%,transparent)]">
        ...
        <div className="absolute top-0 left-0 w-full h-full z-10">
          <div className="flex flex-col items-center justify-center p-6 text-center h-full w-full">
            ...
            <span className="text-lg font-semibold text-white">{displayTitle}</span>
            <p className="text-gray-400 text-sm mt-1 max-w-65">{displayDescription}</p>
          </div>
        </div>
      </div>
